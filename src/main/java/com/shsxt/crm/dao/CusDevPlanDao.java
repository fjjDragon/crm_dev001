...
/**
 * Created by fjj on 2017/8/14.
 */
public interface CusDevPlanDao {

    @Select("select t.id, t.exe_affect, t.plan_date, t.plan_item, t.sale_chance_id from t_cus_dev_plan t where sale_chance_id= #{saleChanceId} and is_valid = 1")
    List<CusDevPlan> findBySaleChanceId(@Param(value="saleChanceId") Integer saleChanceId);

    @Insert("insert into t_cus_dev_plan (plan_item, plan_date, exe_affect,create_date, update_date, is_valid, sale_chance_id) values(#{planItem}, #{planDate},#{exeAffect}, now(), now(), 1, #{saleChanceId})")
